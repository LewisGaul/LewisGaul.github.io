...
This is partly because I have less complaints with `black` than `pylint`, but more for consistency between projects since there are some differences in the formatting of `black` and `ruff`, and I expect most people to have their IDE set up to run `black`.
The `ruff` formatter has known deviations listed at <https://docs.astral.sh/ruff/formatter/black/>.
Many of these look sensible, although I'm not a fan of the difference omitted there but mentioned at <https://docs.astral.sh/ruff/formatter/#format-suppression>, that "`# fmt: on` and `# fmt: off` comments are enforced at the statement level", since I often use this with `subprocess.run()` something like:
```python
subprocess.run(
    [
        # fmt: off
...
If `uv` was installed using the `curl` script, then to update you can simply run `uv self update`.
...
The `_version.py` file is excluded because it is generated by `hatch-vcs`, which gets the version from the project's git tags.
